...
def main():
    ...
    model = get_model()
    checkpointer = ModelCheckpoint(filepath="dataset-chord-normalized-40-test.hdf5", verbose=1, save_best_only=True)
    history = LossHistory()
    result = model.fit(np.asarray(train_data_set), np.asarray(train_label_set), batch_size=10,
                       epochs=30, verbose=1, validation_data=(np.asarray(test_data_set), np.asarray(test_label_set)),
