...
def main():
    ...
    pickle.dump(results, open('dct_velocity_result.pkl', 'wb'))
    # pass


def show_evaluation_plot():
    results = np.load('dct_velocity_result.pkl')
    results = np.asarray(results)
    plt.figure(figsize=(10, 6))
    plt.plot([8,10,12,15,20,40], results[:, 0], lw=2, marker='o', c='r', markersize=12)
    ...
    plt.tight_layout()
    plt.savefig('./dct_velocity_auc.pdf')
    plt.figure(figsize=(10, 6))
    plt.plot([8,10,12,15,20,40], results[:, 1], lw=2, marker='o', c='r', markersize=12)
    plt.xlabel('Dct Coefficient Number', fontdict={'style': 'normal', 'weight': 'bold', 'size': 22})
    plt.ylabel('EER', fontdict={'style': 'normal', 'weight': 'bold', 'size': 22})
    plt.xticks(fontsize=20, fontname='normal')
    plt.yticks(fontsize=20, fontname='normal')
    plt.tight_layout()
    plt.savefig('./dct_velocity_eer.pdf')
    plt.show()
